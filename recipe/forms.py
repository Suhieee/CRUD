...
class RecipeForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'description': forms.Textarea(attrs={'class': TEXT_AREA, 'placeholder': "Enter description...", 'rows': '3'}),
            'cook_time': forms.TextInput(attrs={'class': INPUT_CLASSES, 'placeholder': 'Enter cooking time'}),
            'serving': forms.TextInput(attrs={'class': INPUT_CLASSES, 'placeholder': 'Enter how many people it can serve'}),
        }
...
class InstructionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'step_number': forms.TextInput(attrs={'class': INPUT_CLASSES, 'placeholder': 'Enter step number', 'type': 'number'}),
            'instruction_text': forms.TextInput(attrs={'class': INPUT_CLASSES, 'placeholder': "Enter instruction"}),
        }
...
class IngredientForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'name': forms.TextInput(attrs={'class': INPUT_CLASSES, 'placeholder': 'Enter a name'}),
            'metric': forms.Select(attrs={'class': SELECT_AREA}),
            'quantity': forms.TextInput(attrs={'class': INPUT_CLASSES, 'placeholder': "Enter quantity"}),
        }
